...
function EtherSweepUI() {
	...
	function ICOScreen(id) {
		function Whitepaper() {
			...
			var ico = "<p class='ico_header'>ICO</p><p>The main goal of ICO is to raise funding for the promotion of the project, which will give you profit in the future. By buying " + symbol + " coins you are buying access to the contract's balance, which consists of funds collected from ICO and 5% commission from each event's reward. More bettors - more profit.</p><p>Tokens can be generated only while ICO is going on. Its price linearly increasing from $0.30 to $1.00 within 30 days. Contract's owner getting 1.5x tokens that you bought. After ICO these tokens should not be more than 30% of the total supply, so half of them will be withdrawn and spent on the promotion and development of the project.</p>";
			var token = "<p class='ico_header'>Token</p><p>Token is a ticket that allows you to withdraw your stake of ETH from the available contract's balance. Your stake calculates by dividing your " + symbol + " balance on total supply of tokens.</p><p>Available contract's balance is a balance that contains commission only from finished events. You can get it's value any time by calling availableBalance() function on the contract.</p><p>Withdraw destroys tokens.</p>";
		...
		$("body").fadeOut(500, function() {
			...
			switch(state) {
				...
				case 1: {
					...
					metamaskScreen.content("Getting Started").html("<p>1. Install MetaMask</p><p>2. Refresh page</p>");
